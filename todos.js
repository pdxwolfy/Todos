...
app.use((req, res, next) => {
  res.locals.store = new PgPersistence(req.session); // create a new data store
  next();
});
...
app.post("/lists",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if (!created) {
        req.flash("Failed to create todo list.");
      } else {
...
app.post("/lists/:todoListId/todos",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let created = await res.locals.store.createTodo(todoTitle, +todoListId);
...
app.post("/lists/:todoListId/edit",
  ...
  catchError(async (req, res) => {
    ...
    try {
      ...
      let updated = await res.locals.store.setTodoListTitle(+todoListId, req.body.todoListTitle)
